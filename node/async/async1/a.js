...
if (typeof JSON !== 'object') {
	Function('return this')().JSON = {stringify: function(obj){
...
function stringify(obj, indentStr) {
	...
	function stringify2(obj, indentStr, level, cumStr) {
		...
		if (obj.constructor === Array) {
			...
			var str = '[ ' + cumStr +
				stringify2(obj[0], indentStr, level, cumStr);
			if (obj.length === 1) return str + ' ]';
			for (var i = 1; i < obj.length; ++i)
				str += ', ' + cumStr +
					stringify2(obj[i], indentStr, level, cumStr);
			return str + ' ]';
		}

		try {
			var keys = Object.getOwnPropertyNames(obj); //Object.keys(obj);
		} catch (e) { var keys = []; }
		...
		for (var i = 1; i < keys.length; ++i) {
			...
			str += ', ' + cumStr + JSON.stringify(keys[i]) + ': ' +
				stringify2(y, indentStr, level, cumStr);
		}
		return str + ' }';
	}
}
if (typeof process === 'object')
	console.log('process =', stringify(process, '  '));
if (typeof window === 'object')
	console.log('window =', stringify(window, '  '));
